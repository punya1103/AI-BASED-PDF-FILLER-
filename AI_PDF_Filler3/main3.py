...
def show_popup(message):
    ...
    messagebox.showinfo("PDF Form Filler", message)
    root.destroy()

def main():
    # Load the knowledge database from the text file
    extracted_data = load_data_from_text('data/new_data.txt')

    # Open the PDF form and fill it
    mapped_data = map_fields_to_pdf(extracted_data)
    output_pdf = 'output/filled_form.pdf'
    fill_pdf_form('data/new_dummy_form.pdf', output_pdf, mapped_data)

    # Show a pop-up after filling the PDF
    show_popup(f"PDF form has been successfully filled and saved as: {output_pdf}")
